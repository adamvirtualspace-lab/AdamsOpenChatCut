...
| Transcript | Transcription jobs (AssemblyAI cloud or local/offline Whisper via whisper.cpp / transformers.js), SRT/VTT import, word-level editing, pause compression, search, speakers, and clip views |
...
## Local Transcription (offline, GPU-optional)

This fork adds a fully local transcription path alongside the existing AssemblyAI cloud provider. It is opt-in and backwards-compatible — a fresh clone still transcribes with zero extra setup, and AssemblyAI is unchanged.

**Two engines, auto-selected** via `WHISPER_ENGINE` (`auto` | `onnx`):

- **whisper.cpp** — used when installed. GPU-capable (CUDA / Vulkan), runs `large-v3` / `large-v3-turbo`, and ships Silero VAD. Roughly 20× realtime on a discrete GPU.
- **transformers.js** (`@huggingface/transformers`, bundled) — the zero-install fallback. CPU-only and downloads its own model, so nothing breaks without whisper.cpp installed.

Both engines share one provider/language resolver (used by the Transcript-panel button and the agent's `transcribe_track` tool), report live progress (percent / position / ETA) at `/api/transcribe-local/progress`, and pass through the configured spoken language instead of assuming one.

Transcript quality handling: anti-repetition on the ONNX path, optional pre-ASR denoise, `--max-len` + `--split-on-word` segmentation for whisper.cpp, a repeat-verification pass that re-transcribes suspicious repeat runs in isolation, and preserved cue boundaries so the transcript renders as short lines rather than one block. Settings can download whisper.cpp models from the official ggml repo.

### Other additions in this fork

- **SRT / VTT import** — attach an existing subtitle file as a word-level transcript (`import_transcript` agent tool) instead of re-transcribing.
- **Transcript UI** — a delete-transcript action and a one-click "make captions from the current transcript" button; caption tracks can now be deleted.
- **i18n** — the app no longer always starts in Chinese (it follows the browser language), and agent replies plus proposal-card labels are localized.

### Transcription settings

| Key | Values | Purpose |
|---|---|---|
| `TRANSCRIPTION_PROVIDER` | `assemblyai` / `whisper` | ASR backend |
| `TRANSCRIPTION_LANGUAGE` | ISO-639-1, blank = auto | spoken language |
| `WHISPER_ENGINE` | `auto` / `onnx` | whisper.cpp when present, or force bundled |
| `WHISPER_MODEL` | `tiny`…`large` | bundled (ONNX) model size |
| `WHISPER_DENOISE` | `1` / blank | pre-ASR denoise (ONNX path) |
| `WHISPER_CPP_DIR` / `_BIN` / `_MODEL` / `_VAD_MODEL` | paths | whisper.cpp locations (default `~/whisper.cpp`) |
| `WHISPER_CPP_MAX_LEN` | chars, `0` = off | segment length cap (default 18) |
